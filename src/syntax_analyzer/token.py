...
Several token ids are different from reference.
":=" : GETS -> assign
".." : THRU -> to
"**" : EXPO -> exp
...
Typical usage example:
    ...
    new_tok = Token("i", "id")
    new_tok = Token("semicolon expected", "err")
    new_tok = Token("eof", "eof")
    tok_str = str(Token("+"))
    lit_str = tok_to_lit("l_par")
    tok_id = lit_to_tok(";")
"""
...
TOK_ID_DICT["assign"] = ":="  # DIFFERENT - GETS
TOK_ID_DICT["comma"] = ","
TOK_ID_DICT["type"] = "type"
TOK_ID_DICT["range"] = "range"
TOK_ID_DICT["to"] = ".."  # DIFFERENT - THRU
TOK_ID_DICT["l_par"] = "("
TOK_ID_DICT["r_par"] = ")"
...
TOK_ID_DICT["and"] = "and"
TOK_ID_DICT["or"] = "or"
TOK_ID_DICT["exp"] = "**"  # DIFFERENT - EXPO
TOK_ID_DICT["not"] = "not"
TOK_ID_DICT["eq"] = "="
TOK_ID_DICT["ne"] = "/="
TOK_ID_DICT["lt"] = "<"
...
class Token(object):
    ...
    valid_tok_type: Set[str] = {"int", "id", "type", "err", "eof"}

    def __init__(self, lit: str, tok_type: str = None) -> None:
        """Init with literal code optional token type."""
        self.lit: str = lit
...
# def tok_to_lit(tok_id: str) -> str:
#     """Convert token id into literal code.
#
#     Returns None if given token id is not present.
#
#     Args:
#         tok_id: A string of token id.
#
#     Returns:
#         A string of literal code.
#     """
#     return TOK_ID_DICT[tok_id] if tok_id in TOK_ID_DICT else None


def lit_to_tok(lit: str) -> Token:
    """Convert literal code into Token instance.

    Returns None if given literal code is not present.
    ...
    """
    # return LIT_DICT[lit] if lit in LIT_DICT else None
    return Token(LIT_DICT[lit]) if lit in LIT_DICT else None
