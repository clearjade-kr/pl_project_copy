...
class Parser(object):
    """Parse soruce program and analyze the syntax through recursive descent approach.
    ...
    def __test_token(self, expected: str, err_msg: str) -> None:
        """Test if the current token matches the expected token.

        Args:
            expected: A string representing expected token id.
            err_msg: Error message to be printed if the two tokens do not match.
        """
        if self.token.tok_id != expected:
            self.__raise_error(err_msg)
        self.__next_token()

    def __raise_error(self, err_msg: str) -> None:
    ...
    def compilation(self) -> None:
        ...
        if self.chario.src:
            ...
            self.__test_token("eof", "Unexpected file termination")
            self.chario.report_errors()

    def __subprogram_body(self) -> None:
        self.__subprogram_spec()
        self.__test_token("is", "'is' expected")
        self.__declarative_part()
        self.__test_token("begin", "'begin' expected")
        self.__seq_of_statements()
        self.__test_token("end", "'end' expected")
        if self.token.tok_id == "id":
            self.__next_token()
        self.__test_token("semi", "';' expected")
    ...
    def __obj_num_declaration(self) -> None:
        ...
        self.__test_token("colon", "':' expected")
        if self.token.tok_id == "const":  # numberDeclaration
            self.__next_token()
            self.__test_token("assign", "':=' expected")
            self.__expression()
    ...
    def __identifier_list(self) -> None:
        self.__test_token("id", "identifier expected")
        while self.token.tok_id == "comma":
            self.__next_token()
            self.__test_token("id", "identifier expected")

    def __type_declaration(self) -> None:
        self.__test_token("type", "'type' expected")
        self.__test_token("id", "identifier expected")
        self.__test_token("is", "'is' expected")
        self.__type_definition()
        self.__test_token("semi", "';' expected")
    ...
    def __range(self) -> None:
        self.__test_token("range", "'range' expected")
        self.__simple_expression()
        self.__test_token("to", "'..' expected")
        self.__simple_expression()
    ...
    def __enum_type_definition(self) -> None:
        self.__test_token("l_par", "'(' expected")
        self.__identifier_list()
        self.__test_token("r_par", "')' expected")

    def __array_type_definition(self) -> None:
        self.__test_token("array", "'array' expected")
        self.__test_token("l_par", "'(' expected")
        self.__index()
        while self.token.tok_id == "comma":
            self.__next_token()
            self.__index()
        self.__test_token("r_par", "')' expected")
        self.__test_token("of", "'of' expected")
        self.__name()

    def __subprogram_spec(self) -> None:
        self.__test_token("proc", "'procedure' expected")
        self.__test_token("id", "identifier expected")
        if self.token.tok_id == "l_par":
            self.__formal_part()
    ...
    def __name(self) -> None:
        self.__test_token("id", "identifer expected")
        if self.token.tok_id == "l_par":
    ...
    def __null_statement(self) -> None:
        self.__test_token("null", "'null' expected")
        self.__test_token("semi", "';' expected")

    def __loop_statement(self) -> None:
        if self.token.tok_id == "while":
            self.__test_token("while", "'while' expected")
            self.__condition()
        self.__test_token("loop", "'loop' expected")
        self.__seq_of_statements()
        self.__test_token("end", "'end' expected")
        self.__test_token("loop", "'loop' expected")
        self.__test_token("semi", "';' expected")
    ...
    def __if_statement(self) -> None:
        self.__test_token("if", "'if' expected")
        self.__condition()
        self.__test_token("then", "'then' expected")
        self.__seq_of_statements()
        while self.token.tok_id == "elsif":
            self.__test_token("elsif", "'elsif' expected")
            self.__condition()
            self.__test_token("then", "'then' expected")
            self.__seq_of_statements()
        if self.token.tok_id == "else":
            self.__test_token("else", "'else' expected")
            self.__seq_of_statements()
        self.__test_token("end", "'end' expected")
        self.__test_token("if", "'if' expected")
        self.__test_token("semi", "';' expected")
    ...
    def __exit_statement(self) -> None:
        self.__test_token("exit", "'exit' expected")
        if self.token.tok_id == "when":
            self.__test_token("when", "'when' expected")
            self.__condition()
        self.__test_token("semi", "';' expected")
